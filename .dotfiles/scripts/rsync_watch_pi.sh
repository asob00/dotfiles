...
then 
	...
	inotifywait $paths -m -r -e close_write,moved_to,delete |
		...
		do
			...
			echo "$back_path" > ~/log.txt	
			rsync -av --delete --rsync-path="mkdir -p backup/$back_path && rsync" "$directory" -e ssh "pi@192.168.0.66:backup/$back_path"
			# rsync -av "$directory" –-rsync-path "mkdir -p $back_path && rsync" -e "ssh -i \"$priv_key\"" "$username"@"$remote_host":"backup/$back_path" --delete	
		done
else
